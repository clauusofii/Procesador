...
if archivo is not None:
    try:
        with st.spinner("Procesando reporte..."):
            ...
            df_foodservice = df_export[
                df_export["T. Cliente"].isin(["Foodservice"])
            ].copy()

            # No se filtra por motivo de rechazo: se consideran todos los registros Food Service.
            df_problemas = df_foodservice.copy()
            ...
            cod_locales_afectados = set(
                df_pendientes["Cod Local normalizado"].dropna()
            )

            # Para Food Service se consideran todos los sectores.
            # Por lo tanto, el reporte final incluye todos los productos pendientes.
            df_pedido_completo_pendiente = df_pendientes.copy()
            ...
            with pd.ExcelWriter(salida, engine="openpyxl") as writer:
                ...
                df_foodservice.to_excel(writer, sheet_name="Clientes Foodservice", index=False)
                df_problemas.to_excel(writer, sheet_name="Registros Food Service", index=False)
                df_pendientes.to_excel(writer, sheet_name="Pedidos pendientes", index=False)
        ...
        <div class="result-card">
            ...
            <div style="margin-top: 10px;">
                Clientes Food Service con demanda insatisfecha:
            </div>
            ...
            <div class="small-muted">
                La hoja Re agendar incluye los productos pendientes de todos los sectores.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        metricas = [
            ("📄", len(df_export), "Data original", "SKU's no facturados de todos los sectores y clientes del día n-1"),
            ("👥", len(df_foodservice), "Registros Clientes Food Service", "SKU's no facturados de clientes Food Service"),
            ("📞", cajas_reagendar, "Cajas para reagendar", "Cajas pendientes de todos los sectores"),
            ("🏪", len(cod_locales_afectados), "Clientes para reagendar", "Locales Food Service con productos pendientes"),
        ]
        ...
        with col_a:
            ...
            <div class="card check-list">
                ...
                ✅ Inclusión de todos los motivos de rechazo<br>
                ✅ Identificación de productos pendientes<br>
                ✅ Detección de clientes Food Service con productos no entregados<br>
                ✅ Generación de productos pendientes de todos los sectores<br>
        ...
        with col_b:
            ...
            <div class="card">
                ...
                🚨 Pedidos pendientes de todos los sectores<br>
                🧾 Productos no entregados<br>
                📞 Clientes a quienes reagendar
            </div>
            """, unsafe_allow_html=True)
